// utils/hooks/useSubCategoryData.ts
import { useCallback, useEffect, useState } from "react";
import axiosInstance from "../functions/axios";
import { CMS_URL } from "@/configs";

export const useSubCategoryData = (endpoint: string) => {
  const [sectionData, setSectionData] = useState<SubCategoryInterface | null>(
    null
  );
  ...
  const fetchSectionData = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get<SubCategoryInterface>(url);
