// utils/hooks/useCategoryData.ts
import { useCallback, useEffect, useState } from "react";
import axiosInstance from "../functions/axios";
import { CMS_URL } from "@/configs";

export const useCategoryData = (endpoint: string) => {
  const [sectionData, setSectionData] = useState<CategoryInterface | null>(
    null
  );
  ...
  const fetchSectionData = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get<CategoryInterface>(url);
